...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, hidden_nf, num_atoms, in_edge_nf=0, device='cpu', act_fn=nn.SiLU(), n_layers=7,
                 residual=True, attention=False, normalize=False, tanh=False):
        '''
        :param in_node_nf: Number of features for 'h' at the input
        :param hidden_nf: Number of hidden features
        :param num_atoms: Number of atoms for 'x' at the output
    ...
    def forward(self, h, x, edges, edge_attr):
        ...
        h = self.node_dec(h)
        h = h.view(-1, self.num_atoms, self.hidden_nf)
        h = torch.sum(h, dim=1)
